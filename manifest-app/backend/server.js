...
//
// Bootstrap de la aplicación. Toda la lógica vive en módulos:
//
//   db/connection.js            conexión única a SQLite
//   db/migrations.js            migraciones idempotentes de arranque
//   services/xmlParser.js       manifiestos XML de la DGA
//   services/pdfParser.js       PDF: US Customs 1302 y genérico DGA
//   services/txtGenerator.js    TXT de Hacienda (205 chars por línea)
//   services/blValidation.js    reglas compartidas por export y push
//   services/siscommateClient.js  único punto de contacto con SISCOMMATE
//   routes/*.js                 endpoints agrupados por dominio
//
// El contenido original de este archivo (1601 líneas) queda comentado al final,
// por pedido — no eliminar.
...
// ─── RUTAS ───────────────────────────────────────────────────────────────────
// Cada router declara sus rutas con la ruta completa (/api/...), por lo que se
// montan en la raíz. Las URLs son exactamente las mismas de antes.
app.use(require('./routes/manifests'));
app.use(require('./routes/bl'));
...
// ═══════════════════════════════════════════════════════════════════════════
// CÓDIGO ORIGINAL DE server.js — CONSERVADO COMENTADO, NO ELIMINAR
//
// Este es el contenido exacto del archivo antes de la separación en módulos
// (git HEAD). Se conserva comentado como referencia y respaldo. Cada bloque
// vive ahora en su módulo correspondiente:
//   parseXmlManifest            → services/xmlParser.js
//   parsePdfManifest / 1302     → services/pdfParser.js
//   generateTxtLine0/1/2        → services/txtGenerator.js
//   getBridgeConfig/bridgeRequest → services/siscommateClient.js
//   getDB / migrate             → db/connection.js + db/migrations.js
//   app.get/post/put/delete     → routes/*.js
// ═══════════════════════════════════════════════════════════════════════════
// // server.js — Editor de manifiestos DGA → Hacienda PR + SISCOMMATE bridge
// require('dotenv').config();
// const express  = require('express');
